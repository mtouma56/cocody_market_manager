...
import 'dart:html' if (dart.library.io) 'dart:io' show Blob, Url, AnchorElement, document;
...
class ExcelExportService {
  ...
  Future<void> exporterRapportPaiements({
    ...
    try {
      ...
      buffer.writeln('Période : $periode (${DateFormat('dd/MM/yyyy').format(dateDebut)} - ${DateFormat('dd/MM/yyyy').format(dateFin)})');
      buffer.writeln('Généré le ${DateFormat('dd/MM/yyyy à HH:mm').format(DateTime.now())}');
      buffer.writeln('');
      ...
      final totalPaye = paiements
        ...
        .fold<double>(0.0, (sum, p) => sum + ((p['montant'] as num?)?.toDouble() ?? 0));
